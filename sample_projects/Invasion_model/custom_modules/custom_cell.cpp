...
void Custom_cell::ecm_cell_function (Cell* pCell, Phenotype& phenotype, double dt){

	std::vector<Cell*> neighbors = get_possible_neighbors(pCell);

	Custom_cell* custom_pCell = static_cast<Custom_cell*>(pCell);
	
	Cell* pOtherCell = NULL;

	for( int n=0; n < neighbors.size() ; n++ )
	{
		pOtherCell = neighbors[n]; 
		Custom_cell* custom_pOtherCell = static_cast<Custom_cell*>(pOtherCell);
		// check if it's not me in the vector
		if( custom_pOtherCell != custom_pCell )
		{
			// calculate distance to the cell 
			std::vector<double> displacement = custom_pOtherCell->position;
			displacement -= custom_pCell->position;
			double distance = norm( displacement ); 
			
			double max_distance = custom_pCell->phenotype.geometry.radius + 
				custom_pOtherCell->phenotype.geometry.radius; 
			max_distance *= 1.1; 
			
			// calculate adhesion using custom function

			custom_pCell->phenotype.mechanics.cell_cell_adhesion_strength = custom_pCell->custom_adhesion_function(custom_pCell, custom_pOtherCell, max_distance);

			// calculate repulsion with custom function

			custom_pCell->phenotype.mechanics.cell_cell_repulsion_strength = custom_pCell->custom_repulsion_function(custom_pCell, custom_pOtherCell);
